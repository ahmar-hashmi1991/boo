...
module.exports = {
    ...
    async  addLikeToComment(id, profileId, commentId) {
        try {
          ...
          const existingLike = comment.likes.find(like => like.user.equals(user._id));
          if (existingLike) {
    ...
    async  getSortedCommentsWithFilter(profileId, sortBy, order, filterPersonalityType) {
        try {
          const profile = await ProfileModel.findById(profileId);
          ...
          if (filterPersonalityType === 'all') {
            ...
            allComments.sort((a, b) => {
              if (sortBy === 'likes') {
                return (a.likes.length - b.likes.length) * (order === 'asc' ? 1 : -1);
              } else if (sortBy === 'timestamp') {
            ...
            // Filter comments by personality type
            // Filter comments by personality type
            const filteredComments = profile.comments.filter(comment => comment.personality.some(p => p.type === filterPersonalityType));
          ...
          console.error('Error getting sorted comments with personality filter:', error);
          throw error;
        }
      }
